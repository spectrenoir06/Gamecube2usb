...
#define GC1_PIN 4
#define GC1_PIN_DIR DDRD

#define GC2_PIN 8
#define GC2_PIN_DIR DDRD

// these two macros set arduino pin 2 to input or output, which with an
// external 1K pull-up resistor to the 3.3V rail, is like pulling it high or
// low.  These operations translate to 1 op code, which takes 2 cycles
#define GC1_HIGH DDRD &= ~0x04
#define GC1_LOW DDRD |= 0x04
#define GC1_QUERY (PIND & 0x04)

#define GC2_HIGH DDRD &= ~0x08
#define GC2_LOW DDRD |= 0x08
#define GC2_QUERY (PIND & 0x08)
...
void setup()
{
  ...
  digitalWrite(GC1_PIN, LOW);  
  pinMode(GC1_PIN, INPUT);
  digitalWrite(GC1_PIN, LOW);  
  pinMode(GC1_PIN, INPUT);
...
/*
  ...
  //do {
      ...
      Serial.print("GC1 zero point read: ");
  ...
  */
  ...
  //do {
      ...
      zero_x = gc_status.stick_x;
      zero_y = gc_status.stick_y;
      Serial.print("GC2 zero point read: ");
      Serial.print(zero_x, DEC);
      Serial.print(", ");
      Serial.println(zero_y, DEC);
      Serial.flush();
...
static void init_gc1_controller()
{
...
static void init_gc2_controller()
{
  // Initialize the gamecube controller by sending it a null byte.
  // This is unnecessary for a standard controller, but is required for the
  // Wavebird.
...
static void gc1_send(unsigned char *buffer, char length)
{
    // Send these bytes
    char bits;
...
static void gc2_send(unsigned char *buffer, char length)
{
    // Send these bytes
    char bits;
...
void loop()
{
    ...
    if (status == 0) {
        ...
        Serial.println(" | GC1 controller read error. Trying to re-initialize");
        Serial.flush();
    ...
    if (status == 0) {
        ...
        // set a neutral N64 string
        Serial.print(millis(), DEC);
        Serial.println(" | GC2 controller read error. Trying to re-initialize");
        Serial.flush();
        gc_status.stick_x = zero_x;
        gc_status.stick_y = zero_y;
        // this may not work if the controller isn't plugged in, but if it
        ...
        Gamepad2.buttons((gc_status.data1 & 0x1F) | ((gc_status.data2 & 0x70) << 1) & 0xFFF);
        Gamepad2.xAxis(map(gc_status.stick_x, 0, 0xFF, 0, 0xFFFF));
        Gamepad2.yAxis(-map(gc_status.stick_y, 0, 0xFF, 0, 0xFFFF));
        Gamepad2.rxAxis(map(gc_status.cstick_x, 0, 0xFF, 0, 0xFFFF));
        Gamepad2.ryAxis(-map(gc_status.cstick_y, 0, 0xFF, 0, 0xFFFF));
        Gamepad2.zAxis(map(gc_status.left, 0, 0xFF, 0, 0xFFFF));
        Gamepad2.rzAxis(map(gc_status.right, 0, 0xFF, 0, 0xFFFF));
        Gamepad2.dPad1(tab[gc_status.data2 & 0xF]);
        Gamepad2.write();
        //print_gc_status();
        //delay(1000);
    }
